...
import telebot
import random
bot = telebot.TeleBot("6095797638:AAEqvW3ZCpqGmcuTcVE5ufQs27P_eAWBnjc")
...
def start(message):
        global flag
        bot.send_message(message.chat.id, f"Прівет! Давай поіграем!")
        bot.send_message(message.chat.id, f"Всего в игре {sweets} конфет")
        flag = random.choice(["user", "bot"])
        if flag == "user":
            bot.send_message(message.chat.id, f"Первым ходишь ты")
            controller(message)
        else:
...
def controller(message):
    ...
    if sweets>0:
        if flag == "user":
            ...
            mrk.add(key1)
            mrk.add(key2)
            bot.send_message(message.chat.id, f"Твой ход введите кол-во конфет от 0 до {max_sweet}",reply_markup=mrk)
